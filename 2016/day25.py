...
def compute(instructions, registers, target):
    pc = 0
    output = ""
    while pc < len(instructions) and len(output) < len(target) and output == target[:len(output)]:
...
def solution_a():
    target = "".join(["01" for i in range(50)])
    i = 0
    r = registers.copy()
    r['a'] = i
    c = compute(instructions, r, target)
    while c != target:
        i += 1
        r = registers.copy()
        r['a'] = i 
